...
- Real OAuth to ad platforms (adapter seam plus seeded data instead; named as next step in README).
- A database (all state is deterministic seed data plus cached JSON; persistence is a named next step).
- User accounts and auth (open demo with rate limiting on expensive routes).
...
- A Vercel cron route (`/api/cron/refresh-intel`) wired but env-gated. It demonstrates the n8n schedule going native.
...
- Every live external call (Anthropic, Apify, Firecrawl) has a timeout, an in-memory rate limit, and a committed cached fallback rendered with a visible "cached" badge. The demo never white-screens.
